...
static int osd_compat_cursor(struct fb_info *info, unsigned long arg)
{
	...
	if (copy_from_user(&cursor.set, &ucursor32->set, 3 * sizeof(__u16)))
		return -EFAULT;
	if (get_user(data, &ucursor32->mask))
		return -EFAULT;
	cursor.mask = compat_ptr(data);
	if (copy_from_user(&cursor.hot, &ucursor32->hot, 2 * sizeof(__u16)))
		return -EFAULT;
			    ...
			    unsigned int cmd, unsigned long arg)
{
	...
	/*coverity[result_independent_of_operands] it can be ignored*/
...
{
#ifdef CONFIG_ARM64
	if (pfn_is_map_memory(__phys_to_pfn(rmem->base)))  {
#else
