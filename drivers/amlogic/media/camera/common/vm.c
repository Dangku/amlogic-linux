...
static inline void vm_vf_put_from_provider(struct vframe_s *vf,
		unsigned int vdin_id);
struct vframe_receiver_op_s *vf_vm_unreg_provider(struct vm_device_s *vdevp);
struct vframe_receiver_op_s *vf_vm_reg_provider(struct vm_device_s
		*vdevp);
...
//	ret = device_create_file(vdevp->dev, &dev_attr_dump);
	ret = device_create_file(vdevp->dev, &dev_attr_vm_attr);
...
int uninit_vm_device(struct platform_device *plat_dev)
{
	...
	/* stop_vm_task(vdevp); */
	device_remove_file(vdevp->dev, &dev_attr_vm_attr);
	device_destroy(vm_clsp, MKDEV(MAJOR(vm_devno), vdevp->index));
	cdev_del(&vdevp->cdev);
