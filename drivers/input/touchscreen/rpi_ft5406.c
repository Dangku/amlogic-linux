/*
 *
 * Rpi 7 Touchscreen FT5406 touch driver.
 *
...
extern int rpi_mcu_is_connected(void);
extern int rpi_mcu_screen_power_up(void);
extern int rpi_mcu_set_bright(int bright);

static void rpi_ft5406_work(struct work_struct *work)
{
	struct rpi_ft5406_data *ts_data
			= container_of(work, struct rpi_ft5406_data, ft5406_work);
	struct ts_event *event = &ts_data->event;
			...
			const struct i2c_device_id *id)
{
	...
	ts_data = kzalloc(sizeof(struct rpi_ft5406_data), GFP_KERNEL);
	if (ts_data == NULL) {
	...
	while(!rpi_mcu_is_connected() && timeout > 0) {
		msleep(50);
		timeout--;
	}
	...
	if (timeout == 0) {
		...
		//LOG_INFO("rpi_mcu_screen_power_up");
...
MODULE_DESCRIPTION("RPI 7 Touchscreen FT5406 Touch driver");
