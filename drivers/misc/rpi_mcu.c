/*
 *
 * Rpi board Touchscreen MCU driver.
 *
...
static struct rpi_mcu_data *g_mcu_data;
...
static int init_cmd_check(struct rpi_mcu_data *mcu_data)
{
	int ret;
	char recv_buf[1] = {0};
...
EXPORT_SYMBOL_GPL(rpi_mcu_screen_power_up);
...
EXPORT_SYMBOL_GPL(rpi_mcu_set_bright);

int rpi_mcu_is_connected(void)
{
	return connected;
}
EXPORT_SYMBOL_GPL(rpi_mcu_is_connected);

static int rpi_mcu_probe(struct i2c_client *client,
			const struct i2c_device_id *id)
{
	struct rpi_mcu_data *mcu_data;
	int ret;
	...
	mcu_data = kzalloc(sizeof(struct rpi_mcu_data), GFP_KERNEL);
	if (mcu_data == NULL) {
...
static int rpi_mcu_remove(struct i2c_client *client)
{
	struct rpi_mcu_data *mcu_data = i2c_get_clientdata(client);
	connected = 0;
	kfree(mcu_data);
	return 0;
}

static const struct i2c_device_id rpi_mcu_id[] = {
	{"rpi_mcu", 0},
	{},
};

static struct i2c_driver rpi_mcu_driver = {
	.driver = {
		.name = "rpi_mcu",
	},
	.probe = rpi_mcu_probe,
	.remove = rpi_mcu_remove,
	.id_table = rpi_mcu_id,
};
module_i2c_driver(rpi_mcu_driver);

MODULE_DESCRIPTION("rpi 7 TouchScreen MCU driver");
